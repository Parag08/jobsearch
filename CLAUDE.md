...
- `lib/editorial/` (DESIGN §2 editorial layer), `lib/watchlist/` (DESIGN §4), `lib/stories/` + `lib/onboarding/` (DESIGN §3, ONBOARDING.md), `lib/freeze.ts` + `lib/services/mark-applied.ts` (freeze on send), `lib/adapters/{gemini,groq,anthropic,llm-factory}.ts` (real providers; `createLlm(env)` routes small/premium, `withLedger` logs every call).
