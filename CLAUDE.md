...
- `lib/editorial/` (DESIGN §2 editorial layer), `lib/watchlist/` (DESIGN §4), `lib/stories/` + `lib/onboarding/` (DESIGN §3, ONBOARDING.md), `lib/freeze.ts` + `lib/services/mark-applied.ts` (freeze on send), `lib/adapters/{vercel-gateway,gemini,groq,anthropic,llm-factory}.ts` (real providers; `createLlm(env)` prefers the Vercel AI Gateway when `AI_GATEWAY_API_KEY` is set - one key, every provider, billed to the plan credit - else Gemini/Groq/Anthropic; routes small/premium, `withLedger` logs every call).
- `lib/interview/`: question bank + scoring (behavioural), `case-sheet.ts` / `case-session.ts` / `case-method.ts` (the case interviewer: code owns the path and checks math; the model only talks, and `guardReply` stops it stating figures not in the case). Cases are DATA in `data/interview/cases.json`.
- `lib/db/` is the ONLY place that knows about Supabase or the demo: `getWorkspace()` returns `{db, userId, mode: "live"|"demo"}` - live via @supabase/ssr cookies (RLS applies), demo = FakeDb loaded from data/cvbuilder when `NEXT_PUBLIC_SUPABASE_URL` is unset, null when configured-but-signed-out. Pages and routes call this and nothing else.
- `app/` - landing (`/`, marketing motion allowance, DESIGN §5), `/signin` (+ server actions, `/auth/callback`), `/app` shell (pipeline, application detail, bank, watchlist; server components + server actions, `force-dynamic`), `/api/{jd,cv-diff,brief}` wired to the services.
- **Sandbox note:** `next build` cannot fetch Google Fonts from the cloud sandbox (egress); build there by stubbing `next/font` in layout.tsx temporarily. CI/Vercel build fine.
